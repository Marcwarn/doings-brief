...
function answeredCount(section: PublicDiscoveryPayload['template']['sections'][number], answers: AnswerState) {
  return section.questions.reduce((count, question) => {
    ...
    if (question.type === 'open' && typeof value === 'string' && value.trim()) return count + 1
    ...
    if (question.type === 'likert' && value && typeof value === 'object' && !Array.isArray(value)) {
      const likertValue = value as LikertAnswer
      if (likertValue.agreement && likertValue.importance) return count + 1
    }
    return count
  }, 0)
}
    ...
    const missingAnswer = payload.template.sections
      ...
      .find(question => {
        ...
        if (question.type === 'likert') {
          ...
          const likertValue = value as LikertAnswer
          return !(likertValue.agreement && likertValue.importance)
        }
        ...
        responses: payload.template.sections.flatMap(section => section.questions.map(question => {
          ...
          if (question.type === 'likert') {
            ...
            return {
              questionId: question.id,
              responseType: 'likert',
              likertAgreement: likertValue.agreement ? Number(likertValue.agreement) : null,
              likertImportance: likertValue.importance ? Number(likertValue.importance) : null,
            }
          }
      ...
      <Shell>
        <SimpleCard
          ...
          description={payload.session.responseMode === 'anonymous'
            ? `${payload.session.clientOrganisation || 'Tack för dialogen hittills'}. Här samlar vi in några fördjupande perspektiv anonymt för att förstå nuläge, behov och riktning bättre.`
            : `${payload.session.clientOrganisation || payload.session.clientName}, tack för dialogen hittills. Här samlar vi in några fördjupande perspektiv inför nästa steg.`}
        >
          <button type="button" onClick={() => setStatus('ready')} style={primaryButtonStyle}>
            Börja svara
          </button>
        </SimpleCard>
      </Shell>
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      <header style={{ background: 'var(--text)', color: '#fff' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 24px 56px' }}>
          ...
          <h1 style={{ margin: '26px 0 12px', maxWidth: 640, fontFamily: 'var(--font-display)', fontSize: 'clamp(2.2rem, 4vw, 3.2rem)', lineHeight: 1.1, letterSpacing: '-0.03em' }}>
            {payload.template.introTitle}
          </h1>
          <p style={{ margin: 0, maxWidth: 620, fontSize: 15.5, lineHeight: 1.75, color: 'rgba(255,255,255,0.74)' }}>
            {payload.template.introText}
          </p>

          {payload.session.responseMode === 'anonymous' && (
            <div style={{ marginTop: 18, maxWidth: 560, padding: '14px 16px', borderRadius: 14, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', display: 'grid', gap: 12 }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '30px 24px 72px' }}>
        <div style={{ marginBottom: 22, overflowX: 'auto', scrollbarWidth: 'none' }}>
                  ...
                  >
                    ...
                    {question.type === 'scale' && (
                      <>
                          ...
                          {scaleValues.map(option => {
                            ...
                            return (
                              <button
                                ...
                                style={scaleButtonStyle(selected)}
                              >
                                {option}
                              </button>
                            )
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, fontSize: 11, color: 'var(--text-3)', maxWidth: 280 }}>
                        ...
                        {question.options.map(option => {
                          ...
                          return (
                            <button
                              ...
                              style={choiceButtonStyle(checked)}
                            >
                              {option.label}
                            </button>
                          )
                    ...
                    {question.type === 'likert' && (() => {
                      ...
                      return (
                        <div style={{ display: 'grid', gap: 18 }}>
                          <div>
                            <div style={{ fontSize: 13, color: 'var(--text-3)', marginBottom: 10 }}>Hur mycket håller du med?</div>
                              ...
                              {[1, 2, 3, 4, 5].map(n => {
                                ...
                                return (
                                  <button
                                    key={n}
                                    type="button"
                                    onClick={() => setLikert(question.id, 'agreement', String(n))}
                                    style={scaleButtonStyle(selected, 54)}
                                  >
                                    {n}
                                  </button>
                                )
                          ...
                          {hasAgreement && (
                            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 18, animation: 'fadeIn 0.2s ease' }}>
                              <div style={{ fontSize: 13, color: 'var(--text-3)', marginBottom: 10 }}>Hur viktigt är detta?</div>
                              <div style={{ display: 'flex', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                                {[1, 2, 3, 4, 5].map(n => {
                                  ...
                                  return (
                                    <button
                                      key={n}
                                      type="button"
                                      onClick={() => setLikert(question.id, 'importance', String(n))}
                                      style={scaleButtonStyle(selected, 54)}
                                    >
                                      {n}
                                    </button>
        ...
        <div style={{ marginTop: 24, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '18px 20px' }}>
          ...
          {error && (
            <div style={{ marginTop: 14, borderRadius: 12, border: '1px solid #f2c5d2', background: '#fff3f7', color: '#8e244c', padding: '12px 14px', fontSize: 13.5 }}>
              {error}
            </div>
          )}
        </div>
...
function SimpleCard({ title, description, children }: { title: string; description: string; children?: React.ReactNode }) {
  return (
    <div style={{ width: '100%', maxWidth: 620, background: '#fff', borderRadius: 20, overflow: 'hidden', border: '1px solid var(--border)', boxShadow: '0 24px 64px rgba(16,24,40,0.08)' }}>
      <div style={{ background: 'var(--text)', padding: '28px 28px 24px' }}>
        ...
        <div style={{ fontSize: 13.5, color: 'rgba(255,255,255,0.78)', marginTop: 10, lineHeight: 1.6 }}>{description}</div>
      </div>
      <div style={{ padding: '24px 28px 28px', background: 'var(--surface)' }}>
        {children}
      </div>
    </div>
  )
...
function scaleButtonStyle(selected: boolean, size = 42): React.CSSProperties {
  return {
    width: size,
    height: size,
    borderRadius: 12,
    ...
    fontSize: size > 42 ? 16 : 14,
    fontWeight: 600,
    cursor: 'pointer',
  }
}

function choiceButtonStyle(checked: boolean): React.CSSProperties {
  return {
    borderRadius: 999,
