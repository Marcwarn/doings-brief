...
// Server-side Supabase with service role (to write without auth)
  ...
  const rows = answers.map((a, i) => `
    <tr>
      <td style="padding:14px 24px;border-bottom:1px solid #f0eaf5;vertical-align:top;width:40%;">
        <span style="font-size:11px;font-weight:600;color:#6b2d82;text-transform:uppercase;letter-spacing:.05em;">
          ${i+1}. ${esc(a.label)}
        </span>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 16px;"><tr><td align="center">
    <table width="640" cellpadding="0" cellspacing="0" style="max-width:640px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(107,45,130,0.08);">
      <tr><td colspan="2" style="background:linear-gradient(135deg,#1e0e2e,#6b2d82);padding:28px 32px;">
        <p style="margin:0 0 4px;font-size:12px;color:rgba(255,255,255,0.55);font-weight:600;text-transform:uppercase;letter-spacing:.08em;">Ny brief</p>
        <h1 style="margin:0;font-size:22px;font-weight:700;color:#fff;">${esc(clientName)}</h1>
        <p style="margin:6px 0 0;font-size:12px;color:rgba(255,255,255,0.5);">Inlämnad ${now}</p>
      </td></tr>
      ...
      <tr><td colspan="2" style="padding:20px 32px;background:#f5f4f8;border-top:1px solid #e8d9f0;">
        <p style="margin:0;font-size:12px;color:#606070;text-align:center;">Skickat via <strong>Doings Brief</strong> · doingsclients.se</p>
      </td></tr>
    </table>
  </td></tr></table>
</body></html>`
...
export async function POST(req: NextRequest) {
  try {
    const { sessionId, clientName, consultantId, token, answers } = await req.json()
    ...
    // 3. Get consultant email
    const { data: profile } = await sb
      .from('brief_consultant_profiles')
      .select('email')
      .eq('id', consultantId)
      .single()

    const consultantEmail = profile?.email || 'marcus.warn@doings.se'
    const now = new Date().toLocaleString('sv-SE', { dateStyle: 'long', timeStyle: 'short' })

    // 4. Send email to consultant
