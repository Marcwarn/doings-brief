...
export async function POST(req: NextRequest) {
  try {
    ...
    const results = await Promise.all(sessionIds.map(async (sessionId: string) => {
      ...
      if (session.consultant_id !== user.id) return { sessionId, ok: false, reason: 'obehörig' }
      ...
      const reminderKey = `discovery_reminder:${sessionId}`
      ...
      const [{ data: profile }, { data: template }] = await Promise.all([
        ...
        sb.from('discovery_templates').select('intro_title').eq('id', session.template_id).single(),
      ])

      const senderName = profile?.full_name || 'Doings'
      const discoveryUrl = `${siteUrl}/discovery/${session.token}`
...
<body style="margin:0;padding:0;background:linear-gradient(180deg,#f7f1fb 0%,#f4f2f8 100%);font-family:'DM Sans',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 16px;">
    <tr><td align="center">
      ...
      if (emailError) {
        console.error('discovery remind email error:', JSON.stringify(emailError))
      ...
      if (settingsError) {
        console.error('discovery remind settings upsert error:', settingsError)
    ...
    console.error('discovery remind error:', err)
